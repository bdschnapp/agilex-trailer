#!/usr/bin/env python3
"""
Physical vehicle launch file (with trailer)
...
def generate_launch_description():
    ...
    robot_description_content = Command([
        ...
        PathJoinSubstitution([
            ...
            "description",
            "robot_trailer.urdf.xacro"  # With trailer
        ]),
    ])
    robot_description = {
    ...
    # Joint state publisher for trailer joint
    ...
    # Trailer odometry builder: creates trailer_odom from tractor odom + trailer joint
    trailer_odom = Node(
        package="planning",
        executable="trailer_odom_builder",
        name="trailer_odom_builder",
        output="screen",
        parameters=[{
    ...
    planner_node = Node(
        ...
        condition=IfCondition(LaunchConfiguration("use_planning")),
        remappings=[
            ("~/odom", "/trailer_odom"),  # Use trailer odometry for planning
        ],
        parameters=[
            grid_map_params,
            hybrid_astar_params,
    ...
    mpc_node = Node(
        ...
        condition=IfCondition(LaunchConfiguration("use_planning")),
        remappings=[
            ("~/cmd", "/trailer_cmd"),  # Output to vehicle control
            ("~/odom", "/trailer_odom"),
    ...
    # Control Bridge
    # ========================
    # Convert AckermannDrive (from planner) to Joy messages (for canbridge)
    ...
    # Note: canbridge is already launched in sensors.launch.py
    # It subscribes to /joy and sends CAN messages to the vehicle
